...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const t = messages.Product.daily_ai_scholar;
  ...
  return {
    ...
    alternates: {
      canonical: `/${locale}/daily-ai-scholar`,
    },
  };
}

export default async function DailyAIScholarPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const messages = (await import(`../../../../messages/${locale}.json`)).default;
  const t = messages.Product.daily_ai_scholar;
  const common = messages.Product;
  const bento = messages.BentoGrid.daily_ai_scholar;
        ...
        <div className="container-main">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex flex-wrap gap-4 mb-20">
              <Link href="https://github.com/kaixuebang/Daily-AI-Scholar" target="_blank" className="btn-primary">
                {common.view_source}
              </Link>
            </div>
